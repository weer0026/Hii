 ...
 * the directory that contains the extension class file. When using Hii::t() to translate an extension message,
...
{
	const CACHE_KEY_PREFIX='Hii.CPhpMessageSource.';
	 ...
	 * When using Hii::t() to translate an extension message, the category name should be
	...
	{
		//调用CApplicationComponent::init()：添加行为类（如果有）
		parent::init();
		//设置语言文件所在目录
		if($this->basePath===null)
			$this->basePath=Hii::getPathOfAlias('application.messages');
	}
		...
		if(!isset($this->_files[$category][$language]))
		{
			if(($pos=strpos($category,'.'))!==false)
			{
				...
				if(isset($this->extensionPaths[$extensionClass]))
					$this->_files[$category][$language]=Hii::getPathOfAlias($this->extensionPaths[$extensionClass]).DIRECTORY_SEPARATOR.$language.DIRECTORY_SEPARATOR.$extensionCategory.'.php';
				else
				{
	...
	protected function loadMessages($category,$language)
	{
		...
		if($this->cachingDuration>0 && $this->cacheID!==false && ($cache=Hii::app()->getComponent($this->cacheID))!==null)
		{
